...
def register():
    try:
        if request.method == "POST":
            ...
            password_confirm_hash = hash_pw(password_confirm)
            if password_hash == password_confirm_hash:
                c = mysql.connection.cursor()
                c.execute("insert into users (name, email, password) values(%s, %s, %s)", (username, email, password_hash))
...
def login():
    try:
        if request.method == "POST":
            ...
            if user and check_pw(password, user[3]) and check_e(email, user[2]):
                session["user"] = user
...
def check_pw(user_name_password, db_p):
    try:
        return hash_pw(user_name_password) == db_p
    except Exception as e:
        app.logger.error(f' Error: {str(e)}')
        pass

def check_e(user_name_email, db_e):
...
def get_messages_for_room(room_number):
    try:
        c = mysql.connection.cursor()
        c.execute("select users.name, message.MessageText from users inner join message on users.id = message.SenderID where message.RoomID = %s;", (room_number,))
        msg_post = c.fetchall()
        c.close()
        if msg_post:
...
def message_user(data):
    try:
        ...
        c = mysql.connection.cursor()
        c.execute("insert into message(SenderID, MessageText, RoomID) values(%s, %s, %s)", (user_name[0], message, room_number))
        mysql.connection.commit()
        c.close()
...
def all_messages():
    try:
        if "user" not in session:
            ...
            return redirect(url_for("home"))
        c = mysql.connection.cursor()
        c.execute("select users.name, users.email, message.MessageText, message.RoomID, message.Timestamp from message inner join users on message.SenderID = users.id order by message.RoomID ")
        msg_post = c.fetchall()
        c.close()
        if msg_post:
...
def messages_by_user(name_user):
    try:
        if "user" not in session:
            ...
            return redirect(url_for("home"))
        c = mysql.connection.cursor()
        c.execute("select users.name, users.email, message.MessageText, message.RoomID, message.Timestamp from message inner join users on message.SenderID = users.id where users.name = %s", (name_user,))
        msg_post = c.fetchall()
        c.close()
        if msg_post:
...
def messages_by_room(id):
    try:
        if "user" not in session:
            ...
            return redirect(url_for("home"))
        c = mysql.connection.cursor()
        c.execute("select users.name, users.email, message.MessageText, message.RoomID, message.Timestamp from message inner join users on message.SenderID = users.id where message.RoomID = %s", (id,))
        msg_post = c.fetchall()
        c.close()
        if msg_post:
...
def messages_by_word(w):
    try:
        if "user" not in session:
            ...
            return redirect(url_for("home"))
        c = mysql.connection.cursor()
        c.execute("select message.MessageText from message where message.MessageText like %s", ('%' + w + '%',))
        msg_post = c.fetchall()
        c.close()
        if msg_post:
